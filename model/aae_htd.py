# AAE HyperspectralTargetDetection
...
class AE(torch.nn.Module):
    ...
    def lossFn(self, y_true, y_pred, data_input):
    ...
    def forward(self, x, data_input, with_decoder=True):
        ...
        # loss
        R_loss = torch.sum(torch.sum(self.lossFn(x, y, data_input)))
...
class GAN(torch.nn.Module):
    ...
    def forward(self, z, with_G=True):
        ...
        # discriminator loss
        D_loss_real = torch.mean(F.binary_cross_entropy_with_logits(
            D_real_logits, torch.ones_like(D_real_logits)))
        D_loss_fake = torch.mean(F.binary_cross_entropy_with_logits(
            D_fake_logits, torch.zeros_like(D_fake_logits)))
        D_loss = 0.5 * (D_loss_real + D_loss_fake)
        # generator loss
        G_loss = torch.mean(F.binary_cross_entropy_with_logits(
